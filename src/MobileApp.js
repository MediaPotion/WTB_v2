...
function MediaPreviewPopup({ isVisible, eventData, onAdd, onCancel }) {
  if (!isVisible || !eventData) return null;

  // Simple function to get media for an event - now references external database
  const getMediaForEvent = (eventName) => {
    return mediadatabase[eventName] || { photos: [], videos: [] };
  };

  const media = getMediaForEvent(eventData.event);
  const hasMedia = media.photos.length > 0 || media.videos.length > 0;
      ...
      >
        ...
        <div style={{ marginBottom: "20px" }}>
          <p style={{ color: "#666", fontSize: "14px", margin: "0 0 10px 0" }}>
            Duration: <strong>{eventData.duration} minutes</strong>
          </p>
        </div>
        ...
        {!hasMedia ? (
          ...
          >
            ...
            <p>No sample media available for this event type.</p>
            <p style={{ fontSize: "14px", marginTop: "10px" }}>
              This event will still be added to your timeline.
            </p>
          </div>
        ) : (
          <div>
                ...
                >
                  {media.photos.map((photo, index) => (
                    ...
                    >
                      <img
                        src={photo}
                        alt={`${eventData.event} example ${index + 1}`}
                        style={{
                          width: "100%",
                          height: "120px",
                          objectFit: "cover",
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          >
            ...
            <button
              onClick={() => {
                const newValue = row.isOutdoor ? false : true;
                onChange(index, "isOutdoor", newValue);
                onBlur(index);
              }}
              style={{
                width: "48px",
                ...
                fontSize: "10px",
                border: "1px solid #333",
                background: row.isOutdoor ? "#87CEEB" : "#FFE4B5",
                color: "#333",
                cursor: "pointer",
                borderRadius: "4px",
                fontWeight: "bold",
              }}
              title={row.isOutdoor ? "Click for Indoor" : "Click for Outdoor"}
            >
              {row.isOutdoor ? "OUT" : "IN"}
            </button>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  const handlePreviewMedia = (eventData) => {
    setPreviewEventData(eventData);
    setShowMediaPreview(true);
  };

  const handleMediaPreviewAdd = () => {
    handleEventSelect(previewEventData);
  ...
  const exportTimeline = () => {
    ...
    sortedRows.forEach((row) => {
      ...
      timeline += `Setting: ${row.isOutdoor ? "Outdoor" : "Indoor"}\n\n`;
    });
  ...
  return (
    ...
    >
      ...
      <MediaPreviewPopup
        isVisible={showMediaPreview}
        eventData={previewEventData}
        onAdd={handleMediaPreviewAdd}
